...
import { useTTSTimer } from '../hooks/useTTSTimer';
...
interface TTSContextType {
    ...
    updateVoice: (newVoice: string | null) => void;
    updatePitch: (newPitch: number) => void;
...
export function TTSProvider({ children }: { children: React.ReactNode }) {
    ...
    const speak = useCallback(async (text?: string) => {
        const textToUse = text || currentText;
        if (!textToUse) return;

        try {
            const textToSpeak = state.remainingText || textToUse;
    ...
    }, [currentText, state.remainingText, settings, startTimer, stopTimer]);

    const pause = useCallback(async () => {
        try {
            isPausingRef.current = true;

            const remainingText = calculateRemainingText(currentText, state.position);
    ...
    }, [stopTimer, state.position, currentText]);
    ...
    const resume = useCallback(async () => {
        try {
            ...
            if (!textToResume) {
                textToResume = calculateRemainingText(currentText, state.position);
            }
    ...
    }, [currentText, state.remainingText, state.position, state.seekPosition, settings, startTimer, stopTimer]);
    ...
    const seek = useCallback((newPosition: number) => {
        ...
        const remainingText = calculateRemainingText(currentText, newPosition);
    ...
    }, [currentText, state.isPlaying, settings, startTimer, stopTimer]);
    ...
    const updateSpeed = useCallback((newSpeed: number) => {
        ...
        if (state.isPlaying || state.isPaused) {
            ...
            setTimeout(() => {
                ...
                const remainingText = calculateRemainingText(currentText, currentPosition);
    ...
    }, [state.isPlaying, state.isPaused, state.remainingText, currentText, settings.pitch, settings.volume, settings.voice, startTimer, stopTimer]);
